...
- PDF: `pypdf` ile sayfa bazlı metin çıkarımı; taranmış PDF, OCR, tablo ve çok sütunlu okuma sırası garanti edilmez. Şifreli PDF'ler ve 50 milyon karakteri aşan extraction fail-closed reddedilir.
- DOCX: XML içinden paragraf ve temel tablo satırı metni; hücreler `|` ile ayrılır. Görsel, dipnot ve stil/konum fidelity'si garanti edilmez. ZIP üye yolları POSIX/Windows traversal, absolute path ve NUL karakterine karşı doğrulanır; duplicate/symbolic-link üyeler, şifreli veya 50 MB üzeri açılmış paketler fail-closed reddedilir.
- HTML: yerel dosyada görünür metin; `script`, `style`, `nav`, `header`, `footer` ve `aside` alanları atılır. Dış bağlantı takip edilmez.
