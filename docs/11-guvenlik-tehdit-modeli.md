...
| Zararlı dosya veya aşırı corpus taraması | Kaynak başına 10 MB sınır; `ingest-dir` ve corpus inventory için varsayılan 5.000 destekli dosya/1 GB toplam byte kabul kapısı ve limit aşımında embed/store veya inventory parse/yazımı başlamadan reddetme; kaynak kökü dışına yönlenen symlink reddi, DOCX arşiv üye yolu/açılmış boyut/4.096 üye sınırı, şifreli PDF reddi ve PDF extraction sınırı. Üye adedi sınırı ZIP merkez dizininin önceden belleğe alınmasını önlemez; bu kontroller parser process timeout/peak-RAM sandbox'ı veya malware taraması sağlamaz. |
