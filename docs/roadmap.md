...
1. Geniş kişisel corpus ve parser dispatch: parser (Markdown/plain-text, PDF, DOCX, HTML, JSON/JSONL, YAML/YML, RTF, CSV, EML ve XML), metinsiz corpus inventory/manifest, deterministik toplu ingest ve ham-byte duplicate inceleme sinyali uygulandı; PDF için gerçek `pypdf` minimal sentetik smoke testi eklendi. HTML gizli-blok eşleşmesi `html-v4`, DOCX sekme/satır sonu fidelity'si, nested-table yinelenme önlemi ve DTD/entity reddi `docx-v4` kapsamında. Kişisel inventory/manifest varsayılanları Git-ignore altındadır; gerçek/çeşitli PDF corpus kalite kararı final kapısında.
2. En az 300 etiketli sorgu için fixture/veri sözleşmesi, metadata destekli loader, Qdrant gerektirmeyen `fixture-coverage`, opsiyonel label kapsama raporu, shard merge akışı ve split/provenance kontrolü yapan final kabul kapısı hazırlandı; corpus/fixture parser sürüm kümesi binding’i de fail-closed doğrulanıyor. Sentetik corpus için 66 aday sorgu 12 kaynağa yayılmış; temsilî kişisel kayıt dosyasının 11 ve medium günlüğün 14 chunk'ının tümü hedeflenmiştir. Tüm label'lar review-required, 300+ ve large bucket kapıları açıktır.
...
4. Negative sorgular için abstention ve skor eşiği davranışı: opt-in yapılandırılabilir eşik ve `no_candidates`/`below_min_score` privacy-safe telemetrisi eklendi; değer kalibrasyonu final ölçümünde yapılacak.
5. RAG cevap kalitesi değerlendirme sınırı: retrieval/generation ayrımı, context/provenance sözleşmesi ve fail-closed generation handoff policy uygulandı; cevap ölçümü 300+ fixture sonrasına bırakıldı. Benchmark sonuçları fixture/corpus/model provenance kimliklerini de taşır.
6. Harici embedding provider adapter'ı ve kontrollü kullanım yolu: OpenAI adapter'ı opt-in, uzak endpoint'lerde HTTPS (yalnızca loopback HTTP test istisnası), sınırlı retry/backoff, yapılandırılabilir batch bölme, ham metin saklamayan bounded cache ve privacy-safe cache sayaçları ile fail-closed olarak eklendi; hit oranı, maliyet ve kalite etkisi final çalışmasında ölçülecek.
