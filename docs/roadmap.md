...
2. En az 300 etiketli sorgu için fixture/veri sözleşmesi, metadata destekli loader, Qdrant gerektirmeyen `fixture-coverage`, opsiyonel label kapsama raporu, shard merge akışı ve split/provenance kontrolü yapan final kabul kapısı hazırlandı; corpus/fixture parser sürüm kümesi ve chunk-size binding'i de fail-closed doğrulanıyor. Final `fixture-validate` tüm ready kapılarından sonra da label dosyası, güncel corpus manifesti veya tam chunk boyut provenance'ı yoksa reddeder; hazırlık `fixture-coverage` bunlar olmadan çalışabilir. 300 sorgu tek sahipli self-review ile tamamlanabilir; çoklu insan annotator şartı yoktur. Kargo Gemisi DOCX corpus'u eklendikten sonra 66 aday sorgu eski 12 kaynağa bağlı kalacak şekilde checksum/parser provenance'ına yeniden bağlandı; yeni corpus'taki 9 DOCX ve large kaynak henüz sorgu kapsamına alınmadı. Relevance yargılama protokolü binary chunk kanıtını, aday pooling'i ve gerçek-negative doğrulamasını açıklar. Tüm label'lar review-required, 300+ ve large bucket kapıları açıktır.
